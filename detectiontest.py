...
nyquist = 0.5
#filterConfig = generateBandpass(nyquist, 1./(15*24*60), 1./15, 1./(15*24*60), 60.0, 3.0)
#samples = signal.lfilter(filterConfig[0], filterConfig[1], samples)

resultCount = sampleCount/2
...
xticks_coords = [
        mapFreq(nyquist, resultCount, 1./(15)),
        mapFreq(nyquist, resultCount, 1./(30)),
        mapFreq(nyquist, resultCount, 1./(60)),
        mapFreq(nyquist, resultCount, 1./(60*6)),
        mapFreq(nyquist, resultCount, 1./(60*12)),
        mapFreq(nyquist, resultCount, 1./(60*24)),
        mapFreq(nyquist, resultCount, 1./(60*24*1.75)),
        mapFreq(nyquist, resultCount, 1./(60*24*3.5)),
        mapFreq(nyquist, resultCount, 1./(60*24*7)),
        mapFreq(nyquist, resultCount, 1./(60*24*14)),
        mapFreq(nyquist, resultCount, 1./(60*24*15)),
        mapFreq(nyquist, resultCount, 1./(60*24*30))]

xticks_labels = ["0.25h", "0.5h", "1h", "0.25d", "0.5d", "1d", "1.75d", "3.5d", "7d", "14d", "15d", "30d"]



dataCount = mapFreq(nyquist, resultCount, 1./(60*6))
fft = numpy.dual.fft(samples)
magnitude = numpy.abs(fft[1:dataCount])
phase = numpy.angle(fft[1:dataCount])
...
#ax = fig.add_subplot(3, 1, 2, sharex = fftax)
#ax.plot(phase)
...
plt.xticks(xticks_coords, xticks_labels)
plt.grid()
plt.plot(magnitude)

#plt.plot(phase, color=(1.0, 0.0, 0.0, 1.0))
plt.savefig("/tmp/plot.png", format="png", dpi=72)
plt.show()
